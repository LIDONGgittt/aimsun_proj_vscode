...
extern "C" {
    ...
    /*! Deprecated. Please use ANGConnGetObjectIdByType
    ...
    */
    ...
    /*! Return the object id having the same name, ANGConUnknownObject otherwise. If deleteUShortName is true, then
        name parameter will be deleted.
        The typeName allows to filter the search by object type. If NULL is provided, it returns the first object that matches the name.
    */
    AAPIEXPORT int ANGConnGetObjectIdByType( const unsigned short * name, const unsigned short * typeName, bool deleteUShortName  );
